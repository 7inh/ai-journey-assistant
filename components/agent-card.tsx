...
export default function AgentCard({ agent, isProminent = false, onSelect }: AgentCardProps) {
  const FallbackIcon = () => <span className="text-2xl font-semibold">{agent.name.substring(0, 1).toUpperCase()}</span>
    ...
    >
      <CardHeader
        className={cn(
          ...
          isProminent ? "md:w-1/3 md:border-r" : "flex-row md:flex-col text-center md:items-center",
        )}
      >
        <Avatar className={cn("h-16 w-16 md:h-24 md:w-24", isProminent && "md:h-28 md:w-28")}>
        ...
        <div className={cn(!isProminent && "md:mt-4 text-left md:text-center")}>
          <CardTitle className={cn("text-xl", isProminent && "md:text-2xl")}>{agent.name}</CardTitle>
      ...
      >
        <CardDescription className={cn("flex-grow mb-4", isProminent && "md:text-base")}>
          {agent.description}
        </CardDescription>
        {agent.tags && agent.tags.length > 0 && (
          <div className={cn("flex flex-wrap gap-2 mb-4", !isProminent && "justify-center md:justify-start")}>
            {agent.tags.map((tag) => (
        ...
        <Link href={`/chat?agentId=${agent.id}`} passHref legacyBehavior>
          <Button

...
const PricingTierDisplay: React.FC<{ tier: UsagePricingTier }> = ({ tier }) => (
  <div className="p-2 border rounded-md bg-muted/50">
    {tier.tierName && (
      <p className="font-semibold text-xs mb-0.5">{tier.tierName}</p>
    )}
    <p className="text-xs">
    ...
    {tier.freeQuota && (
      <p className="text-xs text-green-600 dark:text-green-500">
        First {tier.freeQuota} units/month free
      </p>
    )}
    {tier.description && (
...
export default function AgentInformationSidebar({
  agent,
}: AgentInformationSidebarProps) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent className="space-y-3">
        ...
        {agent.latestVersion && agent.version !== agent.latestVersion && (
          <InfoItem
            icon={GitBranch}
            label="Latest"
            value={`${agent.latestVersion} (Update available)`}
          />
        )}
        <InfoItem
          icon={CalendarDays}
          label="Last Updated"
          value={
            agent.lastUpdatedDate
              ? new Date(agent.lastUpdatedDate).toLocaleDateString()
              : "N/A"
          }
        />
        <InfoItem
          icon={UserSquare}
          label="Developer"
          value={agent.developerName || "Unknown"}
        />
        ...
        {agent.billingType === "contact-sales" && (
          <InfoItem
            icon={HelpCircle}
            label="Pricing"
            value="Contact sales for enterprise pricing details."
          />
        )}

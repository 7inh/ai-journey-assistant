...
import { Download, CheckCircle, Palette, Briefcase, ExternalLink } from "lucide-react"
...
export default function AgentHeader({ agent, isInstalled, onInstallToggle, isInstallPending }: AgentHeaderProps) {
    ...
    <header className="mb-8 md:mb-12 pt-4">
      {agent.bannerImageUrl && (
        <div className="relative w-full h-40 sm:h-48 md:h-64 lg:h-80 rounded-lg overflow-hidden mb-4 sm:mb-6 shadow-lg">
          <Image
            src={agent.bannerImageUrl || `/placeholder.svg?width=1200&height=400&query=${agent.name}+Banner`}
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:gap-4 md:gap-6">
        <Avatar className="h-20 w-20 sm:h-24 sm:w-24 md:h-32 md:w-32 rounded-lg border-4 border-background shadow-md sm:-mt-12 md:-mt-16 ml-2 sm:ml-0 flex-shrink-0">
          <AvatarImage
            src={agent.avatarUrl || `/placeholder.svg?height=128&width=128&query=${agent.name.charAt(0)}`}
            alt={agent.name}
          />
          <AvatarFallback className="text-3xl sm:text-4xl">{agent.name.substring(0, 1).toUpperCase()}</AvatarFallback>
        </Avatar>
        <div className="mt-3 sm:mt-0 flex-1">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight">{agent.name}</h1>
          ...
          {agent.averageRating !== undefined && agent.ratingCount !== undefined && (
        ...
        >
          {isInstallPending ? <Spinner className="mr-2 h-5 w-5" /> : <ButtonIcon className="mr-2 h-5 w-5" />}
          {buttonText}
        </Button>
      </div>
    </header>

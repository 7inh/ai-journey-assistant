...
interface AgentListItemProps {
  agent: Agent
  rank?: number
  onInstallToggle: (agentId: string, isCurrentlyInstalled: boolean) => Promise<void>
    ...
    <Link href={`/explore/${agent.id}`} className="block group w-full">
      <div
        className={cn(
          ...
          !hideBottomBorder && "border-b border-border/80 dark:border-border/50",
          className,
        )}
      >
        {rank && <span className="text-sm font-medium text-muted-foreground w-8 text-center">{rank}</span>}
        <div className={cn("flex-shrink-0 w-12 h-12 sm:w-14 sm:h-14 relative", rank ? "ml-2" : "ml-0")}>
          <Image
        ...
        <div className="ml-3 sm:ml-4 flex-grow min-w-0">
          <h3 className="text-sm sm:text-base font-medium truncate group-hover:text-primary">{agent.name}</h3>
          <p className="text-xs sm:text-sm text-muted-foreground truncate">{shortDescription}</p>
          {agent.isPremium && <p className="text-xs text-amber-600 dark:text-amber-500 mt-0.5">Premium</p>}
        </div>
        <div className="ml-3 sm:ml-4 flex-shrink-0 flex flex-col items-center">
          <Button
            ...
            aria-label={agent.isInstalled ? `Uninstall ${agent.name}` : `Install ${agent.name}`}
          >
            {isInstallPending ? (
            ...
            <span className="ml-1">{isInstallPending ? "" : agent.isInstalled ? "Remove" : "Get"}</span>
          </Button>

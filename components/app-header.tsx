...
import { Settings, LogOut, UserCircle, Search, ChevronDown, ArrowLeft } from "lucide-react"
...
export default function AppHeader() {
  ...
  const [exploreSearchTerm, setExploreSearchTerm] = useState(searchParams.get("q") || "")
  ...
  const handleGlobalKeyDown = useCallback(
    (event: KeyboardEvent) => {
      ...
      if (event.key === "j" && (event.metaKey || event.ctrlKey)) {
        event.preventDefault()
        const newJourneyNavItem = sidebarNavItems.find((item) => item.id === "new-journey")
          ...
          toast({
            ...
            description: (
              <div>
                ...
                {agent.releaseNotes && <p className="text-xs mt-1">Changes: {agent.releaseNotes}</p>}
              </div>
            ),
          })
  ...
  const isAgentDetailPage = pathname.startsWith("/explore/") && pathname.split("/").length === 3
  ...
  const renderCenterContent = () => {
    if (overrideTitle) {
      return (
        <h1 className="text-lg font-semibold truncate px-1 sm:px-2" title={overrideTitle}>
          {overrideTitle}
        </h1>
      )
    }
    ...
    if (isChatPage) {
      ...
      const agentForDisplay = agentsData.find((agent) => agent.id === currentActiveAgentId)

      if (
        !currentActiveAgentId ||
        (currentActiveAgentId && !installedAgents.find((agent) => agent.id === currentActiveAgentId))
      ) {
        const defaultJourneyMaster = installedAgents.find((agent) => agent.id === DEFAULT_AGENT_ID)
        const firstInstalledAgent = installedAgents.length > 0 ? installedAgents[0] : undefined
        ...
        if (currentActiveAgentId && currentActiveAgentId !== searchParams.get("agentId")) {
          const params = new URLSearchParams(searchParams.toString())
      ...
      const finalAgentForDisplay = agentsData.find((agent) => agent.id === currentActiveAgentId)
      const availableAgentsToSwitch = installedAgents.filter((agent) => agent.id !== currentActiveAgentId)
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              ...
              disabled={availableAgentsToSwitch.length === 0 && !finalAgentForDisplay}
            >
              <span className="truncate max-w-[100px] xs:max-w-[120px] sm:max-w-[180px] md:max-w-[250px] lg:max-w-[300px] xl:max-w-[350px]">
                {finalAgentForDisplay?.name || (agentsData.length > 0 ? "Select Agent" : "Loading Agents...")}
              </span>
    ...
    if (isAgentDetailPage && agentForTitle) {
      return (
        <h1 className="text-lg font-semibold truncate px-1 sm:px-2" title={agentForTitle.name}>
          {agentForTitle.name}
        </h1>
      )
    }
    ...
    return (
      <h1 className="text-lg font-semibold p-1 h-auto truncate" title={getPageTitle()}>
        {getPageTitle()}
      </h1>
    )
  }
    ...
    >
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        {!isMobile && sidebarDesktopState === "collapsed" && (
          <SidebarTrigger className="flex-shrink-0" aria-label="Open sidebar" />
        )}
        {isAgentDetailPage && (
          <Button
            variant="ghost"

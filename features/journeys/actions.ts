...
import { getJourneyById, saveJourney, type JourneyLogItemType } from "./api" // Corrected import path for types
import { findSuitableAgentForTask } from "./utils"
import type { Journey, JourneyLogItem, TaskData, PhaseData } from "@/interfaces"
...
const addLogEntryToJourney = (
  ...
  details: Partial<Omit<JourneyLogItem, "id" | "journeyId" | "type" | "timestamp">> = {},
): Journey => {
...
export async function createJourney(originalRequest: string): Promise<Journey> {
  try {
    ...
    const newJourneyData: Omit<Journey, "log"> = {
      id: `journey-${Date.now()}`,
      title: originalRequest.substring(0, 50) + (originalRequest.length > 50 ? "..." : ""),
    ...
    let journeyWithLog = addLogEntryToJourney({ ...newJourneyData, log: [] }, "journey-start", {
...
export async function updateJourneyStatus(journeyId: string, status: string): Promise<Journey> {
  try {
    ...
    console.error(`Error updating journey ${journeyId} status to ${status}:`, error)
    throw error instanceof Error ? error : new Error(String(error))
  }
}

export async function addStepToJourney(journeyId: string, stepDescription: string): Promise<Journey> {
  try {
    ...
    const newTask: TaskData = {
      ...
      completed: false,
      assignedAgentId: findSuitableAgentForTask(stepDescription)?.id || DEFAULT_AGENT_ID,
    }

    if (!journey.phases || journey.phases.length === 0) {
      const newPhase: PhaseData = { id: `phase-${Date.now()}`, name: "General Tasks" }
      journey.phases = [{ ...newPhase, tasks: [newTask] }]
      journey = addLogEntryToJourney(journey, "phase-header", { phaseData: newPhase, isCurrentSystemPlan: true })
    } else {
      journey.phases[0].tasks.push(newTask)
    }
    journey = addLogEntryToJourney(journey, "task-definition", { taskData: newTask, isCurrentSystemPlan: true })
    journey = addLogEntryToJourney(journey, "system-message", { text: `New step/task added: "${stepDescription}"` })
  ...
  if (lowerContent.startsWith("add task:")) {
    ...
    if (taskName) {
      const newTask: TaskData = {
        ...
        name: taskName,
        completed: false,
        assignedAgentId: findSuitableAgentForTask(taskName)?.id || DEFAULT_AGENT_ID,
      }
      if (!journey.phases || journey.phases.length === 0) {
        const newPhase: PhaseData = { id: `phase-${Date.now()}`, name: "Tasks" }
        journey.phases = [{ ...newPhase, tasks: [newTask] }]
        journey = addLogEntryToJourney(journey, "phase-header", { phaseData: newPhase, isCurrentSystemPlan: true })
      } else {
        journey.phases[0].tasks.push(newTask)
    ...
    journey = addLogEntryToJourney(journey, "system-message", { text: `Received request to process: ${content}` })
...
export async function updateJourneyTask(
    ...
    tasks: phase.tasks.map((task) => {
      if (task.id === taskId) {
        ...
        if (updateType === "toggleComplete") {
          ...
          journey = addLogEntryToJourney(journey, "system-message", {
            text: `Task "${taskNameForLog}" marked as ${updatedTask.completed ? "complete" : "incomplete"}.`,
          ...
          if (decisionText) {
            journey = addLogEntryToJourney(journey, "system-message", {
              text: `Decision "${decisionText}" for task "${taskNameForLog}" was ${approved ? "approved" : "set to not approved"}.`,
              relatedTaskId: taskId,
            })
          }
        }
  ...
  if (!taskUpdated) {
    console.warn(`Task with ID ${taskId} not found in journey ${journeyId} for update.`)

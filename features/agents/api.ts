...
import { mockAgents } from "./mock-agents" // This import should now work
...
export async function fetchAgentDetails(agentId: string): Promise<Agent | undefined> {
  await apiDelay(100)
  const agent = mockAgents.find((agent) => agent.id === agentId)
  return agent ? JSON.parse(JSON.stringify(agent)) : undefined
}

export async function fetchRelatedAgents(currentAgent: Agent): Promise<Agent[]> {
  await apiDelay(100)
  if (!currentAgent.relatedAgentIds || currentAgent.relatedAgentIds.length === 0) {
    return []
  }
  const related = mockAgents.filter((a) => currentAgent.relatedAgentIds!.includes(a.id) && a.id !== currentAgent.id)
...
export async function submitAgentReview(
  ...
  const newAverageRating = ((agent.averageRating || 0) * (agent.ratingCount || 0) + rating) / newRatingCount

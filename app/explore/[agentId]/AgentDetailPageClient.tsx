...
import { fetchAgentDetails, fetchRelatedAgents, installAgent, submitAgentReview } from "@/features/agents/api"
...
export default function AgentDetailPageClient({ params }: { params: { agentId: string } }) {
  const [agent, setAgent] = useState<Agent | null>(null)
  const [relatedAgents, setRelatedAgents] = useState<Agent[]>([])
  const [isInstalled, setIsInstalled] = useState(false)
  const [isFetchingInitialData, setIsFetchingInitialData] = useState(true) // Renamed from isLoading
  const [isInstallPending, startInstallTransition] = useTransition() // For install/uninstall button
  const { toast } = useToast()
  // const { addToCart, isAgentInCart } = useCart() // Add this
  ...
  const handleInstallToggle = async () => {
    if (!agent || agent.isInstalled || agent.billingType === "contact-sales") {
      ...
      if (agent?.isInstalled) {
        toast({ title: "Already Active", description: `${agent.name} is already active.` })
      }
      return
    }
  ...
  if (isFetchingInitialData) {
    // This will be handled by app/explore/[agentId]/loading.tsx
    return null
  }
      ...
      <div className="container mx-auto py-6 sm:py-8 px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 lg:gap-x-8 xl:gap-x-12 gap-y-8 mt-8 md:mt-12">
          <main className="lg:col-span-2 space-y-8 md:space-y-10">
            <AgentAboutSection description={agent.longDescription || agent.description} />
            {agent.showcaseItems && agent.showcaseItems.length > 0 && (
              <AgentShowcaseSection items={agent.showcaseItems} />
            )}
            <AgentReviewsSection agent={agent} onSubmitReview={handleReviewSubmit} />
          </main>

          <aside className="space-y-6 md:space-y-8 lg:pt-1">
            <AgentInformationSidebar agent={agent} />
            {relatedAgents.length > 0 && <AgentRelatedSidebar relatedAgents={relatedAgents} />}
          </aside>
        </div>
      </div>
    </div>
